...
#
# scripts/setup.sh -- one-time provisioning so Elenchon can load.
# Idempotent. Does NOT install SBCL/Quicklisp -- checks and tells you how.
#   1. Clone Coalton (local checkout; Elenchon's typed core is Coalton-first).
...
# Coalton local checkout (Elenchon's typed core).
...
# Discovery: run `sbcl --dynamic-space-size 4096 --script bootstrap.lisp` at the repo root (writes the ASDF drop-in).

if [ "${1:-}" = "--test" ]; then
  info "Loading Elenchon (first run compiles Coalton; a few minutes)..."
  sbcl --dynamic-space-size "${ELENCHON_DYNAMIC_SPACE_SIZE:-4096}" --non-interactive \
    --eval "(load \"$QL_HOME/setup.lisp\")" \
    --eval '(handler-case (ql:quickload :elenchon) (error (e) (format *error-output* "~&FAILED: ~A~%" e) (uiop:quit 1)))'
  info "Loaded."
fi
info "Setup complete. Next: run 'sbcl --dynamic-space-size 4096 --script bootstrap.lisp' at the repo root, then (ql:quickload :elenchon) from any REPL."
