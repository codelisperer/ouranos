# Elenchon — Editor Setup

Get Claude plus the Common Lisp / Coalton dev stack working in VS Code, Emacs, or
Neovim. This mirrors the shared setup across the six-framework ecosystem, in
dependency order: `aion` → `cons` → `mnemosyne` → `elenchon` → `hyperion` →
`praxeon`.
...
3. **Launch VS Code from a terminal** — `cd elenchon && code .`. VS Code inherits
...
4. The committed `.vscode/settings.json` overrides Alive's `startCommand` to
   **load the system into the LSP image on start** — so `(in-package #:elenchon)`
   resolves and completion/xref see the real code (no *"does not designate any
   package"* nag). **First boot is slower**: it loads Elenchon and compiles Coalton
...
- **`Heap exhausted, game over`** on the first load — Coalton's first compile needs
  more than SBCL's 1 GB default. The repo-root `bootstrap.lisp` is run with
  `--dynamic-space-size 4096`; a bare hand-started `sbcl` needs the flag too.
- `.vscode/alive/` (Alive's per-project compile scratch) is gitignored — don't
  commit it.

## See also
- `docs/method.md` (the design brief) · `docs/user-guide.md` · `docs/roadmap.md` ·
  `docs/elenchon-vision.md` · `CLAUDE.md`.
