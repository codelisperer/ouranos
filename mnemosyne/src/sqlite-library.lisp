...
dladdr cannot place it."
  ...
  (cffi:with-foreign-object (info :pointer 4)
    (when (zerop (cffi:foreign-funcall "dladdr" :pointer address :pointer info :int))
...
GetModuleFileNameW. Signals an error if either call fails."
  ...
  (let ((capacity 32768))
    (cffi:with-foreign-objects ((module :pointer) (buffer :uint16 capacity))
      ...
      (let ((n (cffi:foreign-funcall "GetModuleFileNameW"
...
path is unknown, and why, is the result they need to print."
  (handler-case
      (let ((address (%symbol-address +probe-symbol+)))
        (if (null address)
            ...
            (list :path (%file-containing address)
                  :version (cffi:foreign-funcall "sqlite3_libversion" :string)
                  :error nil)))
    (error (e)
      (list :path nil :version nil :error (princ-to-string e)))))
