...
be shown catching the loss it exists to catch. Windows, both packagings. **Linux** has the same end-to-end check in
`scripts/verify-appimage-update.sh` (#251): a real AppImage installed, a real signed update applied
through the manifest, the new file started, `~/.<appname>` byte-identical, and a `--control` that
leaves the new file non-executable and must fail. **macOS is blocked on §7's unwritten strategy
(#251) and is openly unrun** — a guarantee kept
